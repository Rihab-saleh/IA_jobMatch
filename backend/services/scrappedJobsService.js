...
// Update the /api/jobs/search route to include the API job counts
app.post('/api/jobs/search', async (req, res) => {
  try {
    const filters = req.body;
    const { jobs, apiJobCounts } = await searchJobs(filters);
    res.json({ success: true, count: jobs.length, apiJobCounts, jobs });
  } catch (error) {
    console.error('Error searching jobs:', error);
...
// Create job route (requires authentication in a real app)
app.post('/api/jobs', async (req, res) => {
  try {
    const { title, company, location, description, salary, url } = req.body;
    
    // Validation
    if (!title || title.length < 3) {
      return res.status(400).json({ success: false, error: 'Title must be at least 3 characters long.' });
    }
    
    if (!company || company.length < 3) {
      return res.status(400).json({ success: false, error: 'Company must be at least 3 characters long.' });
    }
    
    if (!location || location.length < 3) {
      return res.status(400).json({ success: false, error: 'Location must be at least 3 characters long.' });
    }
    ...
    if (!salary || salary.length < 3) {
      return res.status(400).json({ success: false, error: 'Salary must be at least 3 characters long.' });
    }
    
    if (!url || url.length < 3) {
      return res.status(400).json({ success: false, error: 'URL must be at least 3 characters long.' });
    }
    ...
    res.json({ 
      ...
      job: { title, company, location, description, salary, url }
    });
  } catch (error) {
