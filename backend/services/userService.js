...
class UserService {
  ...
  async handlePublicReactivateRequest(identifier, reason) {
    ...
    try {
      ...
      const user = await User.findOne({ person: person._id }).populate("person").lean()
  ...
  async updateProfilePicture(userId, pictureData) {
    ...
    // Stocker uniquement l'URL (chemin) de l'image et l'ID public
  ...
  async getCertifications(userId) {
    const profile = await Profile.findOne({ userId }).populate("certifications").exec()
  ...
  async deleteCertification(userId, certificationId) {
    ...
    if (profile) {
      profile.certifications = profile.certifications.filter((certId) => certId.toString() !== certificationId)
      await profile.save()
    }
  ...
  async updateLanguage(userId, languageId, languageData) {
    const language = await Language.findOneAndUpdate({ _id: languageId, user: userId }, languageData, { new: true })
