...
async function getRecommendationsForUser(userId) {
  try {
    ...
    // Assuming preferences are part of the userProfile or fetched separately if needed
    // If separate: const userPreferences = await userService.getUserPreferences(userId); 

    // 2. Fetch jobs (you might need to adjust the query based on your needs)
    ...
    console.log("Llama2 response:", content);
